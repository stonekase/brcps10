...
public class brcps_databasequery {
	...
	public static int moveTransactionToPassed(long msisdn,long transaction_id,int transaction_status)
	{
		int response = 0;
	...
	public static int moveTransactionToFailed(long transactinID)
	{
		int numRowsChanged = 0;
		conn = connectionpgsql.getCon();
		try {
			conn.setAutoCommit(false);
			String sql ="INSERT into tbl_transactions_passed(msisdn,amount,transaction_time,transaction_status,transaction_id,cashout_amount) "
					+ "select msisdn,amount,transaction_time,transaction_status,transaction_id,cashout_amount from tbl_transactions_pending where transaction_id="+transactinID;
			//create the prepared statement to fight against injection to my code
			PreparedStatement pst =  conn.prepareStatement(sql);
			numRowsChanged = pst.executeUpdate();
			pst.close();
